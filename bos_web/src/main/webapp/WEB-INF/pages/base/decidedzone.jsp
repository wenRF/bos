...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length<1){
			$.messager.alert('提示信息','请选中一个关联区!','info')
		}else{
			//给全局id属性赋值
			id = rows[0].id;
			//单选 选中之后点击出选择框
			...
			url="${pageContext.request.contextPath}/decidedzoneAction_findUnassociatedCustomers.action"
			//使用ajax显示未关联的客户信息，显示到左边下拉框
			...
			//使用ajax查询关联当前选中定区的客户信息，显示到右边的下拉框中
			var url = "${pageContext.request.contextPath}/decidedzoneAction_findAssociatedCustomers.action";
			//先清除下拉框中的数据
			$('#associationSelect').empty()
	...
	function doClickRow(rowIndex, rowData){
		$('#association_subarea').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "${pageContext.request.contextPath}/subareaAction_findByDecidedzoneId.action?decidedzoneId="+rowData.id,
			columns : [ [{
				field : 'id',
				title : '分区编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "${pageContext.request.contextPath}/decidedzoneAction_findAssociatedCustomers.action?id="+rowData.id,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="关联客户窗口" id="customerWindow" collapsible="false" closed="true" minimizable="false" maximizable="false" style="top:20px;left:200px;width: 400px;height: 300px;">
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="customerForm" action="${pageContext.request.contextPath }/decidedzoneAction_guanlianCustomer.action" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
					...
					<tr>
						...
						<td>
							...
							<script type="text/javascript">
							$(function(){
								//给向右的按钮添加点击事件，将左边的选中项移动到右边
								$('#toRight').click(function(){
									//将左边的选中项移动到右边
									$('#associationSelect').append($('#noassociationSelect option:selected'));
								});
								//给向左的按钮添加点击事件，将右边的选中项移动到左边
					...
					<tr>
						<td colspan="3"><a id="associationBtn" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-save'">关联客户
						</a> 
						<script type="text/javascript">
						$(function(){
